...
Consider a few examples. Example 1 process interest statement and then what their study describes/explains. Example 2 process interest statement and then what their study describes/explains. Example 3 process interest statement and then they their study describes/explains. 

All of the aforementioned examples are interested in process, but notice they they are talking about different things. Explain the differences. Which is an appropriate representation of process behavior? This is a dilemma/problem where we do not have consensus.

Here is another example to pinpoint the dilemma. Imagine that we are interested in weight gain among US males who are 50 years or older, and to investigate it we conduct three longitudinal studies. **Option A**. In the first, we find and report that the average male gains one pound every year for the rest of this life. In the second, we find and report a positive relationship between male weight and caloric intake across time, and suggest that caloric intake influences weight. In the third, we propose that weight at any point in time is determined by its prior value and the difference between the number of consumed calories and the number of expended calories.
...
Which of these is a process study? Where is the line between explaining the process versus describing the emergent properties of the process?

In this paper, we try to build consensus around this issue. What do we mean by process? What are the different ways we can represent it? Two parts to this paper: 1) clarify what process is, and distinguish between explaining process versus describing process; 2) discuss principles and fundamentals for either explaining or describing process. 
